...
class VisualizeExample:
    ...
    def _build_simulation(self):
        ...
        for i in range(self.num_hand_dofs):
            hand_builder.joint_target_ke[i] = 150.0
            hand_builder.joint_target_kd[i] = 5.0
    ...
    def _load_policy(self, checkpoint_path: str):
        """Load trained policy."""
        num_obs = 43  # 16 + 16 + 3 + 4 + 4
        num_actions = 16

        self.policy = ActorCritic(num_obs, num_actions)
    ...
    def _compute_obs(self) -> torch.Tensor:
        ...
        obs = torch.zeros(self.num_envs, 43, dtype=torch.float32, device=self.torch_device)
        ...
        for i in range(self.num_envs):
            ...
            obs[i, 0:16] = joint_q[q_start:q_end]
            obs[i, 16:32] = joint_qd[q_start:q_end] * 0.1
    ...
    def step(self):
        ...
        if self.policy is not None:
            ...
            self.actions = torch.randn_like(self.actions) * 0.1
